...
/*
  ...
  mismo buffer
*/
void log_rock(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i = 0;
...
void log_rock2(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){//
	int i = 0;
...
void hell_sqr(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i = 0;
...
void psychedelic_if(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i = 0;
...
void by_60s(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i=0;
...
void fuzzy_dark_pow4(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i = 0;
...
void rare_cuadratic(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i = 0;
...
void random_day(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	srand(time(NULL));
	int mod2;
	mod2 = rand()%7;
...
void by_pass(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i=0;
   	for (i;i<nframes;i++){
		out[i] = equalizer_effect(mdc,out[i],i);
		out[i] = hall_effect(mdc,out[i],i);
...
void mute(dummy_audio_buf out, m_distortion *mdc, dummy_audio_size nframes){
	int i=0;
	for(i;i<nframes;i++){
		out[i] = 0.0;
	}
